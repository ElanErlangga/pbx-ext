...
var _Sources = (() => {
  ...
  var MangaStreamParser = class {
    ...
    parseMangaDetails($, mangaId, source) {
      ...
      const author = $(`span:contains(${source.manga_selector_author}), .fmed b:contains(${source.manga_selector_author})+span, .imptdt:contains(${source.manga_selector_author}) i, tr td:contains(${source.manga_selector_author}) + td`).contents().remove().last().text().trim();
      const artist = $(`span:contains(${source.manga_selector_artist}), .fmed b:contains(${source.manga_selector_artist})+span, .imptdt:contains(${source.manga_selector_artist}) i, tr td:contains(${source.manga_selector_artist}) + td`).contents().remove().last().text().trim();
    ...
    parseChapterDetails($, mangaId, chapterId) {
      const data = $.html();
      const pages = [];
      let obj = /ts_reader.run\((.[^;]+)\)/.exec(data)?.[1] ?? "";
      if (obj == "") {
      ...
      */
      ...
      this.homescreen_sections = {
        "popular_today": {
          ...
          section: createHomeSection("popular_today", "Popular Today", false, import_types2.HomeSectionType.featured),
...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`);
  ...
  var TukangKomik = class extends MangaStream {
    ...
    configureSections() {
      ...
      this.homescreen_sections["popular_today"].selectorFunc = ($) => $("div.bsx", $("h2:contains(Populer Today)")?.parent()?.next());
  ...
  return __toCommonJS(TukangKomik_exports);
})();
